...
namespace RealEstate.Controllers.Properties.Leases.Payments;
...
public class PaymentController(PaymentService service, ILogger<PaymentController> logger) : ControllerBase
{
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] CreateDTO paymentDTO)
    {
        try
        {
            if (paymentDTO == null)
    ...
    public async Task<IActionResult> Update(string paymentId, [FromBody] UpdateDTO paymentDTO)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(paymentId))
