...
[Route("[controller]")]
[ApiController]
public class LocationController(LocationService service, ILogger<LocationController> logger) : ControllerBase
{
    private readonly LocationService _service = service;
    ...
    [HttpPost("add")]
    public async Task<IActionResult> Add([FromBody] CreateDTO propertyLocationDTO)
    {
        try
        {
    ...
    public async Task<IActionResult> Update([FromBody] UpdateDTO propertyLocationDTO, string locationId)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(locationId))
