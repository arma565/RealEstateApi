...
public sealed class Asset
{
    ...
    private bool _gas;
    private bool _phone;
    //•	Collection initialization is invalid:
    //private readonly ICollection<AssetImage>
    //_assetImages = []; and same for _persons.
    //Use new List<T>() or Array.Empty<T>().
    ...
    //•	Naming: prefer OrderId (Id casing)
    //instead of OrderID to follow .NET naming conventions.
    [DefaultValue(0)]
    public int OrderID
    {
            get => _orderID;
    ...
    //•	Using string to store Date/Time is brittle.
    //Prefer DateTime, DateOnly, TimeOnly (NET 6+),
    //or at least make intent explicit.
    public string? Time
    {
        get => _time;
        set => _time = value;
